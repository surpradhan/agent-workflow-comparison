...
**1. `single_step` achieved the highest completion rate** (100.0%).  
It produced a non-empty answer on every task — though "completed" here means the workflow finished, not that the answer was correct (quality score: 0.748).
...
Human approval checkpoints catch errors before they propagate — when the workflow completes, the answer has been reviewed twice. The trade-off is a 44% completion rate; the hard tasks that would produce poor answers are the ones that fail the review gate.
...
- **Latency-sensitive** (real-time UX): stay with `single_step` or `tool_using` (fastest at 13,581 ms avg).
...
*Benchmark run with llama3.1:8b via Ollama on a local M1 Pro 16 GB machine.  All ten workflow patterns are implemented in pure Python using LangChain-compatible abstractions.  Groq (llama-3.3-70b-versatile) is supported as an optional faster backend via `LLM_PROVIDER=groq`.*
